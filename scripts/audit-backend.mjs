...
add('DB constraints allow editor zero values', /perc_aproveitamento >= 0/.test(migration2) && /quantidade_que_faz >= 0/.test(migration2))
add('Production Edge Function requires JWT', /verify_jwt\s*=\s*true/.test(config))
add('Edge Function rejects anon key fallback', /requireAuthenticatedUser/.test(edge) && /SUPABASE_ANON_KEY/.test(edge) && /auth_required/.test(edge))
add('Edge Function authenticates before reading OpenAI key', edge.indexOf('await requireAuthenticatedUser(req)') > -1 && edge.indexOf("Deno.env.get('OPENAI_API_KEY')") > edge.indexOf('await requireAuthenticatedUser(req)'))
